...
(defn find-project-root
  "Given a directory (or the cwd if non is supplied) try to find root of the
   ...
   1.  The first directory containing a `deps.edn` file with a
       `:kmono/workspace` key present
...
(defn find-project-root!
  ...
  ([dir]
   (let [root (find-project-root dir)]
     (when-not root
       (throw (ex-info "Not a Clojure project"
                       {:type :kmono/root-not-found
