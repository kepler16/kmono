...
(deftest some-test
  (let [config (config/load-config "test/fixtures/example_repo")]
    (testing "Should extract aliases and relativize paths"
      (is (= '{:aliases
               {:kmono/package-deps {:extra-deps {kepler16/bar-lib {:local/root "test/fixtures/example_repo/packages/bar"},
                                                  kepler16/foo-lib {:local/root "test/fixtures/example_repo/packages/foo"}}}
